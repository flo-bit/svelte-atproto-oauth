...
Add `@svelte-atproto/oauth/browser` entry — browser-only OAuth flow using `@atcute/oauth-browser-client`. For static-site deployments (GitHub Pages, Cloudflare Pages, etc.) where there's no server runtime: tokens live in browser localStorage, the DPoP key in IndexedDB, and the only thing that needs to be served is a prerendered `oauth-client-metadata.json`.
